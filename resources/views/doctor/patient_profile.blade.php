...
<div class="container py-4">
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            ...
            <a href="{{ route('doctor.messages.with.patient', $patient->id) }}" class="btn btn-primary">Message Patient</a>
        </div>
    </div>
...
<div class="card mb-4">
    <div class="card-header d-flex justify-content-between align-items-center"> {{-- Added d-flex, justify-content-between, align-items-center --}}
        <h5 class="card-title mb-0">Medication Doses</h5>
                        ...
                        <tbody>
                            @foreach ($patient->medicationDoses->sortByDesc('scheduled_at') as $dose)
                                ...
                                <td>{{ $dose->prescription->dosage ?? 'N/A' }}</td>
                                    ...
                                    <td>
                                        <span class="badge {{ $dose->is_taken ? 'bg-success' : 'bg-warning' }}">
                                            {{ $dose->status }}
                                        </span>
                                    </td>
...
{{-- NEW SECTION: Doctor's Notes --}}
        <div class="card mb-4">
            ...
            <div class="card-body">
                ...
                <form action="{{ route('doctor.notes.store', $patient->id) }}" method="POST" class="mb-4">
                    @csrf
                    <div class="mb-3">
                        <label for="note_content" class="form-label">Add a New Note:</label>
                        <textarea class="form-control @error('note_content') is-invalid @enderror" id="note_content" name="note_content" rows="3" placeholder="Type your note here..." required>{{ old('note_content') }}</textarea>
                        @error('note_content')
